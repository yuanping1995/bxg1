...
class Index extends Base
{
	...
	public function login(){
		if(request()->isPost()){
			...
			$pass=input('password');
			$user=	Db::name('admin_user')->where(['adminuser'=>$username,'adminpass'=>$pass])->find();
		}else{
			return $this->fetch();
		}
	}
	...
	public function tuijianselect(){
		...
		//本周开始时间
		$zhouon = strtotime(date("Y-m-d H:i:s",mktime(0, 0 , 0,date("m"),date("d")-date("w")+1,date("Y"))));
		//本周结束时间
		$zhouoff = strtotime(date("Y-m-d H:i:s",mktime(23,59,59,date("m"),date("d")-date("w")+7,date("Y"))));
	
		
		//本周推荐排行
		$benzhou = Db::name('user')->field('uid,uname,icon,recommend,tuijian_q')->where('recommend','not null')->where('ztime','>',$zhouon)->select();
		$file_path = "tuijian.txt";
		$qid=[];
		if(file_exists($file_path)){
			$str = file_get_contents($file_path);
		...
		{
			if(isset($data[$value['recommend']]['num']))
			{
				$data[$value['recommend']]['num']++;
			$data[$value['recommend']]['jine'] = $qid[$r_usera['tuijian_q']] * $data[$value['recommend']]['num'];
			}else{
				if($r_usera = Db::name('user')->field('uid,tuijian_q')->where('uid',$value['recommend'])->where('tuijian_q','>',1)->find())
				{
					$data[$value['recommend']]['num'] = 1;
					$data[$value['recommend']]['qid'] = $value['tuijian_q'];
					$data[$value['recommend']]['mobile'] = $value['recommend'];
					$data[$value['recommend']]['uname'] = $value['uname'];
					$data[$value['recommend']]['icon'] = $value['icon'];
					$data[$value['recommend']]['jine'] = $qid[$r_usera['tuijian_q']] * 1;
					$data[$value['recommend']]['jiesuan'] = (empty($wo = Db::name('staffwages')->where(['uid'=>$value['recommend'],'zhou'=>$zhouon.'_'.$zhouoff])->find())?'未结算':$wo['jiesuan']);
					$data[$value['recommend']]['zhou'] = $zhouon.'_'.$zhouoff;
				}else{
	...
	function tuijianselectgetshang(){
		...
		{
			//上周开始时间
			$zhouon = strtotime(date("Y-m-d H:i:s",mktime(0, 0 , 0,date("m"),date("d")-date("w")+1-7,date("Y"))));
			//上周结束时间
			$zhouoff = strtotime(date("Y-m-d H:i:s",mktime(23,59,59,date("m"),date("d")-date("w")+7-7,date("Y"))));
			$benzhou = Db::name('user')->field('uid,uname,icon,recommend,tuijian_q')->where('recommend','not null')->where('ztime','>',$zhouon)->where('ztime','<',$zhouoff)->select();
		}else{
		//本周开始时间
		$zhouon = strtotime(date("Y-m-d H:i:s",mktime(0, 0 , 0,date("m"),date("d")-date("w")+1,date("Y"))));
		//本周结束时间
	...
	public function welcome()
    {
		...
		// $goods_count=DB::name('goods')->where('dtime','>=',strtotime($date . ' -1 day'))->count();
		

		$goods_count=0;//昨日上传总数
 ...
 public function getorder(){
        ...
        foreach($dataone as $key => $value){
              $dataone[$key]['seller_name']= Db::name('seller')->where('seller_id',$value['seller_id'])->find()['sname'];
          		 if($value['status']==1){
          		 	$dataone[$key]['status']="<font color='red'>待发货</font>";
          		 }elseif($value['status']==2){
          		 	$dataone[$key]['status']="待收货";
          		 }elseif($value['status']==3){
          		 	$dataone[$key]['status']="<font color='green'>待付款</font>";
...
public function findtixian(){
	$keyword=trim(input('keyword'));
	 	$dataone = Db::name('usercredit')->where("id","=","$keyword")->select();
	 	foreach($dataone as $key=>$val){
