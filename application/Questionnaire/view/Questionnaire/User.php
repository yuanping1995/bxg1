...
class User extends \think\Controller {
	...
	public function index() {
		...
		//用户s
		$this -> assign('order1', Db::name('order') -> where(['uid' => $user_key, 'status' => 3]) -> count());
		//待付款
		$this -> assign('order2', Db::name('order') -> where(['uid' => $user_key, 'status' => 1]) -> count());
		//待发货
		$this -> assign('order3', Db::name('order') -> where(['uid' => $user_key, 'status' => 2]) -> count());
		//待收货
		$this -> assign('order4', Db::name('order') -> where(['uid' => $user_key, 'status' => 8]) -> count());
		//待评论
		$this -> assign('order5', Db::name('order') -> where(['uid' => $user_key, 'status' => 6]) -> count());
		//退货中
		$this -> assign('cur5', 5);
		return $this -> fetch();
	}
	...
	public function mySet() {
		...
		//
		$this -> assign('user', Db::name('user') -> where(['uid' => $user_key]) -> find());
		//用户
		return $this -> fetch();
	}
	...
	public function upload_tou() {
		...
		$up_dir = './uploads/touxiang/';
		//目录路径

		if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_img, $result)) {
			$type = $result[2];
			$new_file = $up_dir . date('Ymd') . substr(implode(NULL, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8) . '.' . $type;
			//文件的名字
	...
	public function ajax_order() {
		...
		if ($listRs) {
			foreach ($listRs as $key => $value) {
				$shoplist[$value['oid']][] = array('oid' => $value['oid'], //
				...
				'logo' => $value['logo'], //
				'yunprice' => $value['yunprice'], 'payprice' => $value['payprice']);
			}
			// dump($shoplist);
			// die;
	...
	// 确认收货
	public function confirmReceipt_bk() {
		$oid = input("orderoid");
		$UserModel = new UserModel;
		$user_key = $UserModel -> isLogin();
		$Confirm = Db::name('order') -> where(['uid' => $user_key, 'oid' => $oid]) -> select();
	...
	public function returns() {
		...
		if ($return) {
			foreach ($return as $key => $value) {
				$returnlist[$value['oid']][] = array('logo' => $value['logo'], //
				'seller_name' => $value['sname'], //
				'goods_pic' => $value['pic'], //
	...
	public function evaluate_order() {
		...
		if ( request() -> isAjax()) {
			...
			foreach ($title as $key => $value) {
				$data[$key] = ['goods_id' => $otime[$key]['goods_id'], //商品Id
				...
				'img' => $img, 'isanonymous' => $anonymous[$key] //是否匿名
				];
			}
	...
	public function myorderdetails() {
		...
		if ($listRs) {
			foreach ($listRs as $key => $value) {
				$shoplist[$value['seller_id']][] = array('oid' => $value['oid'], //
				'goods_id' => $value['goods_id'], //
	...
	public function updateAddress() {
		$id = input("addressid");
		$data = ["surname" => input('add_sname'), "province" => input('add_province'), "city" => input('add_city'), "county" => input('add_area'), "detailed" => input('add_detail'), "mobile" => input('add_stel')];
	...
	public function setMobile() {
		...
		$this -> assign('user', Db::name('user') -> where(['uid' => $user_key]) -> field('uid,mobile') -> find());
		//用户
		return $this -> fetch();
	}
	...
	public function paypassnext() {
		...
		if ( request() -> isAjax()) {
			...
			if (!empty($pass) && !empty($newpass) && !empty($repass)) {
				$password = encryt_data(json_encode(['uid' => $user_key, 'pass' => $pass]), config('PrivateKeyFilePath'));
				//密码
				$addpassword = encryt_data(json_encode(['uid' => $user_key, 'pass' => $repass]), config('PrivateKeyFilePath'));
				//新密码
				$user = Db::name('user') -> where('uid', $user_key) -> find();
				if ($user) {
	...
	public function focusware() {
		...
		if ($foculist) {
			foreach ($foculist as $key => $value) {
				$list[$value['seller_id']][] = array('goods_id' => $value['goods_id'], //
				'uid' => $value['uid'], 'sname' => $value['sname'], //
				'mobile' => $value['mobile'], //
				'price' => $value['price'], //
				'pic' => $value['pic'], //
				'seller_id' => $value['seller_id'], 'xprice' => $value['xprice'], 'seller_name' => $value['seller_name'], //
				'logo' => $value['logo'], //
				'city' => $value['city'], 'logo' => $value['logo'], 'follow_count' => Db::name('follow') -> where('goods_id', $value['goods_id']) -> count(), );
			}
	...
	public function userSurvey() {
		...
		if (!empty($pram)) {
			...
			$data = ['uid' => $user_key, 'time' => time(), 'reason' => $prams[0], 'first_time' => $prams[1], 'otime' => $prams[2], 'sex' => $prams[3], 'age' => (int)$prams[4], 'occupation' => $prams[5]];
	...
	public function myWallet() {
		...
		$signin_key = decrypt_data(cookie('user_data'), config('PublicKeyFilePath'));
		//解密用户数据
	...
	public function ajaxmykiting() {
		...
		if ( request() -> isAjax()) {
			...
			if ($data) {
				...
				$shuju = ['time' => time(), 'uid' => $user_key, 'type' => "账户提现", 'money' => $price, 'typeid' => 2, 'total' => $oldtotal - $price, ];
				...
				return json(['flag' => 1, 'msg' => '提现发起成功']);
				//已支付
			} else {
				return json(['flag' => 0, 'msg' => '提现发起失败']);
				//已支付
			}
		}
	}
	...
	public function checkbank() {
		if ( request() -> isAjax()) {
			...
			$data = ['uid' => $user_key, 'crid' => $number, 'cname' => $name, 'crname' => $crname, 'crtype' => $crtype, ];
	...
	public function recharge() {
		...
		$data = ["oid" => input('oid'), "uid" => $user_key, "status" => 2, "payprice" => input('payprice'), "sname" => "会员升级"];
